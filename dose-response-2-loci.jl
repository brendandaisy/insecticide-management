...
# ╔═╡ f9a3dbac-caf9-11f0-873a-bbd27175dd61
using InvertedIndices

# ╔═╡ a4d21aee-a4a5-4c53-8c72-afe6209a7c1b
using LinearAlgebra

# ╔═╡ 0b88f057-401f-4c8c-9ddf-f6a5662aa0db
using Turing

# ╔═╡ acb94534-dea6-47ab-ad94-eaa2e6fed72a
using HDF5, MCMCChainsStorage

# ╔═╡ 973f374b-e0ae-4c21-91b2-904294f6de5b
using Distributions

# ╔═╡ c7564f4e-6580-4189-a493-e76bdd229d01
using Random

# ╔═╡ 6f699078-041b-41a8-8941-38d0e0f5f1a3
using StatsFuns

# ╔═╡ 94f91580-9912-4b97-8245-845a01b1b84f
using SpecialFunctions

# ╔═╡ 34ba89a6-60f4-4580-81cb-ed211681d228
using CategoricalArrays

# ╔═╡ 4acd1003-64d8-4a15-8643-f69ca9be3da0
using CSV, DataFrames, DataFramesMeta

# ╔═╡ 3b3ddd04-4dcd-4464-80df-340079eaf63e
using CairoMakie

# ╔═╡ 7b1479fa-3700-4bef-a999-ddc3fec29e6c
using AlgebraOfGraphics

# ╔═╡ a7f0d3a7-66fc-4a3f-9b62-009242730eea
include("src/Quasinomials.jl")

# ╔═╡ 194ede7b-6e97-44dd-8920-8a7631860f31
set_aog_theme!()

# ╔═╡ c0a1eeee-7fe0-4545-8f20-aee8a2880bb5
dose = CSV.read("data-raw/Silvie data - dose-response insecticide mortality data.csv", DataFrame; normalizenames=true);

# ╔═╡ ad2e13b2-8b33-4ba0-9afa-00ce5a3ca5a4
@chain dose begin
	@subset!(:dose_per_mosq .> 0)
	@rtransform!(:genotype="$(:locus1)-$(:locus2)")
end

# ╔═╡ 931b87b1-1d26-42ff-9351-2e113d173466
@by dose :genotype begin
	:min=minimum(:dose_per_mosq)
	:mean=mean(:dose_per_mosq)
	:max=maximum(:dose_per_mosq)
end

# ╔═╡ fc3340d9-09bd-4972-9cef-3e9e7880ac8d
viz = data(dose) * 
	mapping(:dose_per_mosq => log, :mortality; color=:genotype) * 
	visual(Scatter);

# ╔═╡ 9e4bf757-b766-488a-8fc4-f6d815568ef8
draw(viz)

# ╔═╡ 6f466f59-6535-4b19-8360-422b85e3a0a5
begin
	loci = @select(dose, :locus1, :locus2) |> Matrix
	U = occursin.(r"[IC]", loci)
	V = occursin.(r"(\w)(?!\1)(\w)", loci)
end

# ╔═╡ 32550be3-310d-40bd-ad50-aacd8bf2ff15
begin
	Y = dose.dead
	N = dose.total
...
# ╔═╡ 27ed34ef-b498-46e0-8b24-b306bc8037c6
function latent_risk(i, θ, U, V, X)
	α₀, α, β₀, β, γ = θ
	allele_risk = @. (α + β*X[i]) * γ^(V[i, :]) * U[i, :] 
	α₀ + β₀*X[i] + sum(allele_risk)
end

# ╔═╡ b6c6f1cc-8b2b-4126-a53f-e2de6016e71c
@model function dose_response(y, U, V, X, N)
	if y === missing
        y = Vector{Int}(undef, length(N))
    end
	α₀ ~ Normal(0, 2)
	α ~ filldist(truncated(Normal(0, 2), upper=0), 2)
	β₀ ~ truncated(Normal(0, 2); lower=0)
	β ~ filldist(Normal(0, 1), 2)
	γ ~ filldist(Beta(1, 1), 2)
	...
	η = map(i->latent_risk(i, (α₀, α, β₀, β, γ), U, V, X), 1:length(y))
	r = cdf.(Normal(), η)
...
# ╔═╡ ba540050-32e2-4e30-9a11-ee7b4dc38235
dr_mod = dose_response(Y, U, V, X, N)

# ╔═╡ f961f87b-38e9-4877-9318-655c16a0cac4
ch_fit = sample(dr_mod, NUTS(2000, 0.7), MCMCThreads(), 200, 2)

# ╔═╡ 8fd627da-dcf0-4cd1-bc01-bbf29b6996e7
describe(ch_fit)

# ╔═╡ d18b9542-a089-430a-b170-7455eb64164b
function plot_chains(chn; size=(600, 600))
	params = names(chn, :parameters) 
	
	chain_mapping = mapping(
		params .=> "sample value";
		color=:chain => nonnumeric, row=dims(1) => renamer(params)
	)
	
	plt1 = data(chn) * mapping(:iteration) * chain_mapping * visual(Lines)
	plt2 = data(chn) * chain_mapping * AlgebraOfGraphics.density(;datalimits=extrema)
	
	fig = Figure(;size)
	facet = (linkxaxes=:none, linkyaxes=:none)
	draw!(fig[1, 1], plt1; facet=(;linkyaxes=:none))
	draw!(fig[1, 2], plt2; axis=(ylabel="density",), facet)

	fig
end

# ╔═╡ 8df80280-cd6a-454a-942c-c1e63c0e56f9
plot_chains(ch_fit)

# ╔═╡ 1080f838-cbf0-4ad2-baf9-59072685e97f
function get_hyperpar(ch, param) 
	@chain DataFrame(ch) begin
		@select(Between("α₀", "γ[2]"))
		rename("$(param)[1]" => "1016𝐈", "$(param)[2]" => "1536𝐂")
		stack(Between("1016𝐈", "1536𝐂"))
	end
end

# ╔═╡ 763e46cf-8a65-412a-b415-8ab19d1df262
post_alpha = get_hyperpar(ch_fit, "α")

# ╔═╡ 5076a7b7-83fd-4d2c-ae62-058b155d4571
post_alpha_viz = data(post_alpha) *
	mapping(:value => "allele resistance α", color=:variable => "") *
	visual(Density; alpha=0.8)

# ╔═╡ 3f2548a8-40fb-4c26-aac7-6ff5fbeb1dba
post_gamma = get_hyperpar(ch_fit, "γ")

# ╔═╡ 16183703-e8ba-49ec-8ff2-3a61a54171d8
post_gamma_viz = data(post_gamma) *
	mapping(:value => "heterozygote reduction γ", color=:variable => "") *
	visual(Density; alpha=0.8)

# ╔═╡ 51f1caf7-7792-41b7-a1d0-dc4ce9d9898e
begin
	yhat = stack(reshape(returned(dr_mod, ch_fit), (:, 1)), dims=2)
	dose_pred = @transform dose begin
		:y_mean=map(mean, eachrow(yhat))
		:y_lo=map(x->quantile(x, 0.025), eachrow(yhat))
		:y_hi=map(x->quantile(x, 0.975), eachrow(yhat))
	end
	sort!(dose_pred, :dose_per_mosq)
	# TODO no this is only r, need to use the y code!
end

# ╔═╡ 096a7dde-43bc-4415-a761-35602878bbc5
viz_ppred = data(dose_pred) * 
	(mapping(
		:dose_per_mosq => "log dose per mosquito", 
		:y_mean => "mortality rate"; 
		lower=:y_lo, upper=:y_hi, 
		color=:genotype
	) * 
	visual(LinesFill; alpha=0.5) +
	mapping(:dose_per_mosq, :mortality => "mortality rate", color=:genotype) *
	visual(Scatter; alpha=0.5));

# ╔═╡ 5ba4ab89-53c0-477a-a15c-a316904f4c02
logaxis_format = (
	xscale=log10, xticks=LogTicks(-2.5:0.5:0),
	xgridvisible=true, xminorticksvisible=true,
    xminorticks=IntervalsBetween(10)
);

# ╔═╡ 5ec2b79d-b99b-427b-be0c-e99a9eb23d5a
draw(viz_ppred; axis=logaxis_format)

# ╔═╡ 15fc1fc6-68d4-4754-b651-3f19c476de02
...
manifest_format = "2.0"
project_hash = "5d338eb99a9b4491b77816cc1c9c9784074887b2"
...
git-tree-sha1 = "0acd6ae46f362503759d056a2e0c6842c81e38b0"
...
git-tree-sha1 = "642af9e4f33cfe6930088418da4c2b0d2b9de169"
