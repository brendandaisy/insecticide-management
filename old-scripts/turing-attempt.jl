...
# ╔═╡ cad60ba2-0fe8-11f0-22b1-350746e4853c
using LinearAlgebra

# ╔═╡ 9aec60ba-70e3-411f-ba03-2c27abf7f8e2
using InvertedIndices

# ╔═╡ eb6c3417-7b4e-4b61-a777-6d7555fc05f3
using NamedTupleTools

# ╔═╡ 53a5b914-0153-476e-af18-7ab0fdf93414
using Turing, DynamicPPL

# ╔═╡ dbba5891-f295-432e-b49e-48739e01d505
using Distributions

# ╔═╡ 18d6ca7f-ae71-4193-b59e-45d1d91e2b88
using CSV, DataFrames, DataFramesMeta

# ╔═╡ 1958dd46-84a6-4b73-b151-3d06f4abf95c
using StatsPlots

# ╔═╡ ec58f65e-c64f-4eb3-b3ae-0eebeacfaf1c
using StaticArrays

# ╔═╡ 320eb271-25b5-4ae2-ad79-0a828622041d
using HypertextLiteral

# ╔═╡ a1196b17-0c4b-41ef-adbf-c2c8756d3d8e
using BenchmarkTools

# ╔═╡ 5505f44c-1bb7-4d0a-8634-721d0927a196
import ReverseDiff

# ╔═╡ 68abb91f-4407-43a3-ad70-969b43da3bee
global ℍ = (:VF, :VC, :IC)

# ╔═╡ 1c244c7a-70f5-4ce8-879a-0c192d30bc18
index2geno = Dict([(i, j) => Symbol(i>=j ? "$(ℍ[j])$(ℍ[i])" : "$(ℍ[i])$(ℍ[j])") for i in 1:3, j in 1:3])

# ╔═╡ 960d2f49-7d9b-4788-ae3c-c416417d8ac0
begin
	geno2index = Dict{Symbol, Vector}()
	for (k, v) in index2geno
	    push!(get!(geno2index, v, []), k)
	end
	geno2index
end

# ╔═╡ 4df70feb-c3e4-40c4-bb8e-17d2ab777682
global 𝔾 = collect(keys(geno2index))

# ╔═╡ df64e560-0e23-4f99-bc67-5f84d02ab3d8
...
# ╔═╡ c3bc1ba8-d89b-4a59-b987-e77b383e5dfd
@model function pop_gen_slow(A₀, R, W, num_gen, num_adults=sum(A))
	A = Vector{Matrix}(undef, num_gen)
	X = Vector(undef, num_gen-1)
	A[1] = A₀
	C = fill(0.5, (3, 3))
	C[diagind(C)] .= 1
	for t in 2:num_gen
...
# ╔═╡ ca61867e-1e08-41f4-8937-f19515c17067
function get_draws(ch, svar::Regex)
	...
	genos = collect(keys(geno2index))
	
	@transform! ret begin
		:t = parse.(Int, get_t.(:variable))
		:geno_ind = genos[parse.(Int, get_gind.(:variable))]
	end
end

# ╔═╡ ea1d6de8-edbd-46c8-a509-cb1917bade79
function fitness_weights(h::T, s₁::T, s₂::T) where {T <: Number}
	1 .- SMatrix{3, 3, T}([0 h*s₁ h*s₂;
		  				  h*s₁ s₁ h*s₂;
		  				  h*s₂ h*s₂ s₂])
end

# ╔═╡ 9ce11540-889a-4775-96c5-6403e4892d30
"""
Returns a named tuple with the proportion of individuals with haplotype H from either parent
"""
function prop_haplotype(A::AbstractMatrix)
	map(i -> sum(A[i, Not(i)]) + sum(A[:, i]), 1:size(A, 1)) ./ sum(A)
end

# ╔═╡ 0fe37aa5-3d46-4b7f-95f3-ee6259ec2738
...
# ╔═╡ 70fcdd53-4462-4d33-b9bd-b0a740b6b56e
h = 0.9; s₁ = 0.5; s₂ = 0.8

# ╔═╡ 200dc07f-061d-4ae3-ac9a-3d1c0c2c8653
W = fitness_weights(h, s₁, s₂)

# ╔═╡ 7eb92c30-fc07-4ca4-ad2c-bd6f92506c19
R = SMatrix{3, 3, Int}(fill(1, (3, 3)))

# ╔═╡ 1b3fe288-d7f4-475f-9244-607b13048984
begin
	num_adults = 500
	A₀ = SMatrix{3, 3, Int}(diagm(num_adults .* [0.1, 0.3, 0.6]))
end

# ╔═╡ dccc90cd-1ea5-4fde-9a42-38a6954ade5d
md"""
### Simulate from the inner model
Using 20 generation above parameters, plotting the haplotypes over time
"""

# ╔═╡ 4dafb4a0-92dd-444d-827a-a7fa0117e41d
@btime sample(pop_gen(A₀, R, W, 20, num_adults), Prior(), 100; progress=false);

# ╔═╡ 4dd9f636-2a5a-4204-ac43-5dd7e46715f8
@btime sample(pop_gen_slow(Matrix(A₀), Matrix(R), Matrix(W), 20, num_adults), Prior(), 100; progress=false);

# ╔═╡ c464c506-d640-4531-bbf2-b60a1321e412
begin
	sim = pop_gen(A₀, R, W, 20, num_adults)()
	ntp = namedtuple(:VF, :VC, :IC)
	sim_haplo = DataFrame(ntp.(prop_haplotype.(sim)))
	sim_haplo.t = 1:nrow(sim_haplo)
	sim_haplo = stack(sim_haplo, [:VF, :VC, :IC])
	nothing
end

# ╔═╡ 06f84df0-bcca-4c39-96e0-eadd1575736a
@df sim_haplo plot(:t, :value, group=:variable);

# ╔═╡ be746d74-cb7b-42b8-89f8-80844e96b549
md"""
Now plotting the genotypes over time
"""

# ╔═╡ a2f39a6b-ab25-43b4-91e8-4fe50131a368
begin
	@assert all(@. isapprox(sum(prop_genotype(sim)), 1, atol=0.001))
	ntpg = namedtuple(collect(keys(geno2index)))
	sim_geno = DataFrame(ntpg.(prop_genotype.(sim)))
	sim_geno.t = 1:nrow(sim_geno)
	sim_geno = stack(sim_geno, collect(keys(geno2index)))
	nothing
end

# ╔═╡ 315eeb31-88e8-48de-ae6e-300614444ec4
@df sim_geno plot(:t, :value, group=:variable);

# ╔═╡ 3f9f45c0-429f-4b5e-aee2-188de6a6b704
md"""
### Full model implementation
"""

# ╔═╡ 23051d85-4103-4e02-8d51-3cf3f9dd5978
@model function fit_pop_gen(y, A₀, R, num_gen, num_adults=sum(A₀), num_adults_obs=sum(y))
	h ~ Beta(1, 4)
	s₁ ~ Beta(1, 1)
	s₂ ~ Beta(1, 1)
	W = fitness_weights(h, s₁, s₂)

	sim ~ to_submodel(pop_gen(A₀, R, W, num_gen, num_adults), false)
	sim_geno = prop_genotype.(sim, num_adults)
	for t in 1:num_gen
		y[t] ~ Multinomial(num_adults_obs, sim_geno[t])
	end
end

# ╔═╡ 2633af8c-3fee-4fa7-9aa5-1418bf279223
Markdown.parse("""
Simulate data from full model conditional on the true values of ``h=$h``, ``s_1=$s₁``, and ``s_2=$s₂``

Following Vera-Maloof et al., observed genotypes of 50 adults per generation for 8 generations
""")

# ╔═╡ 49fc9c62-ab37-4e24-866b-8d69a4a0e323
begin
	num_adults_obs = 50
	num_gen = 8
	model_sim = fit_pop_gen(fill(missing, num_gen), A₀, R, num_gen, num_adults, num_adults_obs)
	ch_cond_pri = sample(model_sim | (;h, s₁, s₂), Prior(), 1)
	nothing
end

# ╔═╡ 2c72038d-1ec0-43b0-8740-80ea7bc037a6
sample(model_sim | (;h, s₁, s₂), Prior(), 1)

# ╔═╡ 686c8702-3871-4ed1-bb1d-8181ecc5d9ad
y_sim = get_draws(ch_cond_pri, r"y")

# ╔═╡ 8268a64b-6c63-4030-9f69-1092dec19f74
y_test = @chain y_sim begin
	@subset(:iteration .== 1)
	@select(:value, :t, :geno_ind)
end;

# ╔═╡ a4ddd228-d711-4ae9-9395-b564c5b06f1e
begin
	using Random

	tmod = fit_pop_gen([y_test[!, :value][(6*g) .+ (1:6)] for g in 0:(num_gen-1)], A₀, R, num_gen, num_adults, num_adults_obs)

	@code_warntype tmod.f(
	    tmod,
	    Turing.VarInfo(tmod),
...
# ╔═╡ f28b9517-ae47-46bc-8f01-c293e263faba
function plot_err(ysw, yt, geno::String)
	yswg = @subset(ysw, :geno_ind .== Symbol(geno))
	ytg = @subset(yt, :geno_ind .== Symbol(geno))

	p = errorline(1:8, Matrix{Float64}(yswg[!, 3:end]), lab=geno)
	scatter!(p, 1:8, ytg[!, :value], lab=nothing)
end

# ╔═╡ dbb273a7-070f-4df5-aa19-1a3680227689
begin
	y_sim_wide = unstack(y_sim, [:t, :geno_ind], :iteration, :value)
	
	plts = [plot_err(y_sim_wide, y_test, g) for g in String.(𝔾)]
	plot(plts..., layout=(3, 2))
end

# ╔═╡ 094da844-1a7b-4ce5-942a-aa695e59015b
begin
	init_samp = sample(model_sim, Prior(), 4) |> DataFrame
	init_params = NamedTuple.(eachrow(@select(init_samp, :h, :s₁, :s₂)))
end

# ╔═╡ 1be425cd-439a-4f89-870a-351d2db647fc
# ╠═╡ disabled = true
#=╠═╡
begin
	y_test_vec = [y_test[!, :value][(6*g) .+ (1:6)] for g in 0:(num_gen-1)]
	model_fit = fit_pop_gen(y_test_vec, A₀, R, num_gen, num_adults, num_adults_obs)
	
	ch_fit = sample(
		model_fit, 
		Gibbs(
			# (:h, :s₁, :s₂) => NUTS(;init_ϵ=0.05, adtype=AutoReverseDiff(compile=true)),
			(:h, :s₁, :s₂) => NUTS(0.85; init_ϵ=0.075),
			:X => PG(80)
		),
		# 500,
		MCMCThreads(),
...
# ╔═╡ 8b52b467-1c08-4799-9ff2-cd7659ef3e0e
#=╠═╡
describe(ch_fit)
  ╠═╡ =#

# ╔═╡ 848e11d5-a302-4dd3-ba97-dd6c31be5cda
#=╠═╡
plot(ch_fit, [:h, :s₁, :s₂])
  ╠═╡ =#

# ╔═╡ f0bed3d9-17c4-4328-b3ed-0f16560ea365
#=╠═╡
yhat = get_draws(predict(model_sim, ch_fit), r"y")
  ╠═╡ =#

# ╔═╡ 9c679f0a-87be-49cb-8f75-fa311eaa3163
begin
	yhat_wide = @chain yhat begin
		@transform(@byrow :draw="$(:iteration), $(:chain)")
		unstack(
			[:t, :geno_ind], :draw, :value;
			renamecols=x -> "sample_$x"
		)
	end
	
	plot([plot_err(yhat_wide, y_test, g) for g in String.(𝔾)]..., layout=(3, 2))
	# yhat_smat = Matrix{Float64}(@select(yhat_wide, Cols(r"sample")))
	# errorline(yhat_wide.t, yhat_smat)
	# plot([plot_err(yhat_wide, y_test, g) for g in String.(𝔾)]..., layout=(3, 2))
end

# ╔═╡ 67d5612f-1f9a-436b-952e-688fe077c4fc
md"""
### Example fit to the Vera Maloof data
"""

# ╔═╡ 2107e9ab-7e73-4ae9-8306-ab6d42968c20
vm = CSV.read("data-proc/vera-maloof.csv", DataFrame);

# ╔═╡ 581da454-5bf1-455d-9c53-2470d7b57fd8
vm_long = @chain vm begin
	stack(Between(:VFVF, :ICIC); variable_name="genotype", value_name="count")
	@subset(match.(r"IF", :genotype) .=== nothing)
	@groupby([:site, :rep, :generation])
	@transform(:N=sum(:count))
end

# ╔═╡ d418f82f-84f0-4653-aeaa-81cb690cb26d
begin
	function plot_vm(vml, geno)
		vmlg = @subset(vml, :genotype .== geno)
		cp = collect(cgrad(:matter, 8, categorical=true))'
		p = plot()
		for (k, vm_rep) in pairs(groupby(vmlg, :rep))
			if k.rep == 1
				@df vm_rep plot!(p, :generation, :count, group=:site, linewidth=2, linecolor=cp)
			else
				@df vm_rep plot!(p, :generation, :count, group=:site, linewidth=2, lab=nothing, linecolor=cp)
			end
		end
		title!(p, geno)
	end

	plot([plot_vm(vm_long, String(g)) for g in 𝔾]..., layout=(3, 2))
end

# ╔═╡ 01f858e5-008a-43b4-ba9e-80446b945847
@model function model_vm(y, A₀, R, num_gen, num_site, num_adults=sum(A₀), num_adults_obs=sum(y))
	h ~ Beta(1, 4)
	s₁ ~ Beta(1, 1)
	s₂ ~ Beta(1, 1)
	W = fitness_weights(h, s₁, s₂)
	...
	for rep in eachindex(y)
		sim ~ to_submodel(pop_gen(A₀[rep], R, W, num_gen, num_adults), false)
		sim_geno = prop_genotype.(sim, num_adults)
		for t in 1:num_gen
			y[rep][t] ~ Multinomial(num_adults_obs, sim_geno[t])
		end
	end
end

# ╔═╡ c5bf9456-68c4-4b71-90a5-110813682b9b
begin
	# TODO: prop_genotype order won't match
	for (k, vm_rep) in pairs(group_by(vm_long, ))
end
...
manifest_format = "2.0"
project_hash = "c35f9888daed7c823d4cdcea00de4e931dd20236"
